...
class DataValidator:
    ...
    def validate(
        ...
        ----------
        asset_class : str
            ``"equity"`` (default) or ``"crypto"``.  Crypto mode skips
            weekend/holiday gap detection, uses 365 expected trading days
            per year, and raises the large-move threshold to 80%.
        """
        ...
        self._check_timestamps(df, result, asset_class=asset_class)
        self._check_ohlcv_sanity(df, result)
        self._check_large_gaps(df, result, asset_class=asset_class)
    ...
    def _check_timestamps(
        ...
        if pd.Timedelta(hours=12) < median_delta < pd.Timedelta(days=5):
            # Weekend/holiday gap detection — skip for crypto (trades 24/7)
            ...
            trading_days_per_year = 365 if asset_class == "crypto" else 252
            expected_trading_days = total_calendar_days * trading_days_per_year / 365  # rough estimate
    ...
    def _check_large_gaps(
        ...
        # Crypto regularly sees 10-20% daily moves; use a higher threshold
        threshold = 80.0 if asset_class == "crypto" else self.max_gap_pct
        ...
        if len(big_moves) > 0:
            ...
            result.add_warning(
                f"{len(big_moves)} bar(s) with >{threshold}% price move: "
