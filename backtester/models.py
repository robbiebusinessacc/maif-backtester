...
class BacktestConfig:
    ...
    @staticmethod
    def for_crypto(initial_capital: float = 100_000, **kwargs) -> "BacktestConfig":
        """Config preset for 24/7 crypto markets (365 bars/year)."""
        return BacktestConfig(bars_per_year=365, initial_capital=initial_capital, **kwargs)
    ...
    def for_intraday(
        ...
        **kwargs,
        ...
        """Config preset for intraday data with correct annualization.

        Supported intervals: 1m, 5m, 15m, 30m, 1h, 4h.
        Assumes 6.5 trading hours/day, 252 trading days/year for equities.
        """
        bars_per_day = {
            "1m": 390, "5m": 78, "15m": 26, "30m": 13, "1h": 7, "4h": 2,
        }
        if interval not in bars_per_day:
            raise ValueError(f"Unknown interval '{interval}'. Use: {list(bars_per_day.keys())}")
        bpy = bars_per_day[interval] * 252
        return BacktestConfig(bars_per_year=bpy, initial_capital=initial_capital, **kwargs)
